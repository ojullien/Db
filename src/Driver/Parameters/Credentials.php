...
final class Credentials
{
     ...
     * @return \Ophp\Db\Driver\Parameters\Credentials
    ...
    public function setCredential($path, $user)
    {
        ...
        if ((strlen($sPath) === 0) || (!is_file($sPath))) {
            throw new \Ophp\Db\Exception\InvalidArgumentException('The file path provided is not valid.');
        }
        ...
        if ((strlen($sUser) === 0) || (!isset($aCredentials[$user])) || (!is_array($aCredentials[$user]))) {
            throw new \Ophp\Db\Exception\InvalidArgumentException('The user provided is not valid.');
        }

        // Filter the credentials
        $aCredential = array_intersect_key($aCredentials[$sUser], ['username' => '1', 'password' => '1']);
        if (count($aCredential) != 2) {
            throw new \Ophp\Db\Exception\InvalidArgumentException('The credentials provided are not valid.');
        }
